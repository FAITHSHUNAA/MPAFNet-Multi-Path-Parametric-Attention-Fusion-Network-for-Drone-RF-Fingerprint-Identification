...
class DroneDataset(Dataset):
    def __init__(self, high_csv_files, high_img_dirs, transform=None):
        self.data = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        high_scaled, high_img_path = self.data[idx]
        label = self.labels[idx]
        high_img = Image.open(high_img_path).convert('RGB')
        if self.transform:
            high_img = self.transform(high_img)
        return (torch.tensor(high_scaled, dtype=torch.float32),
                high_img,
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, p_mfp_high, p_dfp_high):
...
class DPSL(nn.Module):
    ...
    def forward(self, high_feat, high_img):
        p_mfp_high = self.mfp_high(high_feat)
        p_dfp_high = self.dfp_high(high_img)
        p_fused, entropy_loss = self.fusion(p_mfp_high, p_dfp_high)
        logits = self.final_fc(p_fused)
        return logits, p_mfp_high, p_dfp_high, entropy_loss
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for high_feats, high_imgs, labels in train_loader:
            high_feats, high_imgs = high_feats.to(device), high_imgs.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits, p_mfp_high, p_dfp_high, entropy_loss = model(
                high_feats, high_imgs
            )
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for high_feats, high_imgs, labels in val_loader:
                high_feats, high_imgs = high_feats.to(device), high_imgs.to(device)
                labels = labels.to(device)
                logits, _, _, _ = model(high_feats, high_imgs)
...
def test_model(model, test_loader, device):
    ...
    attentions = []
    with torch.no_grad():
        for high_feats, high_imgs, labels in test_loader:
            high_feats, high_imgs = high_feats.to(device), high_imgs.to(device)
            labels = labels.to(device)
            logits, _, _, _ = model(high_feats, high_imgs)
...
def main():
    ...
    full_dataset = DroneDataset(high_csv_files, high_img_dirs, transform=transform)

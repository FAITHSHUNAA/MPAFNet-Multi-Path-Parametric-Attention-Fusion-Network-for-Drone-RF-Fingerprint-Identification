...
class DroneDataset(Dataset):
    def __init__(self, low_img_dirs, transform=None):
        self.data = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        low_img_path = self.data[idx]
        label = self.labels[idx]
        low_img = Image.open(low_img_path).convert('RGB')
        if self.transform:
            low_img = self.transform(low_img)
        return low_img, torch.tensor(label, dtype=torch.long)
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for low_imgs, labels in train_loader:
            low_imgs = low_imgs.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits = model(low_imgs)
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for low_imgs, labels in val_loader:
                low_imgs = low_imgs.to(device)
                labels = labels.to(device)
...
def test_model(model, test_loader, device):
    ...
    attentions = []
    with torch.no_grad():
        for low_imgs, labels in test_loader:
            low_imgs = low_imgs.to(device)
            labels = labels.to(device)
...
def main():
    ...
    full_dataset = DroneDataset(low_img_dirs, transform=transform)

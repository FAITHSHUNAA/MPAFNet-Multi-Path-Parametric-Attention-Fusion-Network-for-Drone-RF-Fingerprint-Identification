...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    best_acc = 0.0
    best_model_path = f'/media/zyj/zuoshun/DroneRF/四路特征/对比实验1low+MFP_best_model_fold_{fold}.pth'
    ...
    for epoch in range(num_epochs):
        ...
        for low_feats, labels in train_loader:
            low_feats = low_feats.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits = model(low_feats)
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for low_feats, labels in val_loader:
                low_feats = low_feats.to(device)
                labels = labels.to(device)
    ...
    if fold is not None:
        ...
        plt.tight_layout()
        plt.savefig(f'/media/zyj/zuoshun/DroneRF/四路特征/对比实验1low+MFP_fold_{fold}_train_curves.png')
        plt.close()
...
def test_model(model, test_loader, device):
    model.eval()
    preds, trues = [], []
    attentions = []  # 仍然保留，但不会使用注意力
    with torch.no_grad():
        for low_feats, labels in test_loader:
            low_feats = low_feats.to(device)
            labels = labels.to(device)
...
def main():
    ...
    full_dataset = DroneDataset(low_csv_files)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(trainval_indices, stratified_labels)):
        ...
        plt.savefig(f'/media/zyj/zuoshun/DroneRF/四路特征/对比实验1low+MFP_test_confusion_matrix_fold_{fold+1}.png')
        plt.close()

        report = classification_report(trues, preds, digits=4)
        with open(f'/media/zyj/zuoshun/DroneRF/四路特征/对比实验1low+MFP_test_classification_report_fold_{fold+1}.txt', 'w') as rf:
    ...
    with open('/media/zyj/zuoshun/DroneRF/四路特征/对比实验1low+MFP_final_test_summary.txt', 'w') as f:
        for i in range(10):

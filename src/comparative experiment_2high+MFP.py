...
class DroneDataset(Dataset):
    def __init__(self, high_csv_files):
        self.data = []
        self.labels = []
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for high_feats, labels in train_loader:
            high_feats = high_feats.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits = model(high_feats)
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for high_feats, labels in val_loader:
                high_feats = high_feats.to(device)
                labels = labels.to(device)
...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for high_feats, labels in test_loader:
            high_feats = high_feats.to(device)
            labels = labels.to(device)
...
def main():
    ...
    full_dataset = DroneDataset(high_csv_files)

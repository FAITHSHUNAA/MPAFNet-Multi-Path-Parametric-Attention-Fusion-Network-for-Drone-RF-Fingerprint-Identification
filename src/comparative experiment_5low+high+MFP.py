...
class DroneDataset(Dataset):
    def __init__(self, low_csv_files, high_csv_files):
        self.data = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        low_scaled, high_scaled = self.data[idx]
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, p_mfp_low, p_mfp_high):
...
class DPSL(nn.Module):
    ...
    def forward(self, low_feat, high_feat):
        ...
        p_fused, entropy_loss = self.fusion(p_mfp_low, p_mfp_high)
        logits = self.final_fc(p_fused)
        return logits, p_mfp_low, p_mfp_high, entropy_loss
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for low_feats, high_feats, labels in train_loader:
            low_feats = low_feats.to(device)
            high_feats = high_feats.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits, p_mfp_low, p_mfp_high, entropy_loss = model(
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for low_feats, high_feats, labels in val_loader:
                low_feats = low_feats.to(device)
                high_feats = high_feats.to(device)
                labels = labels.to(device)

                logits, _, _, _ = model(low_feats, high_feats)
...
def test_model(model, test_loader, device):
    ...
    attentions = []
    with torch.no_grad():
        for low_feats, high_feats, labels in test_loader:
            low_feats = low_feats.to(device)
            high_feats = high_feats.to(device)
            labels = labels.to(device)
...
def main():
    ...
    full_dataset = DroneDataset(low_csv_files, high_csv_files)

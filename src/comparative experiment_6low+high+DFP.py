...
class DroneDataset(Dataset):
    def __init__(self, low_img_dirs, high_img_dirs, transform=None):
        self.data = []
        self.labels = []
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, p_dfp_low, p_dfp_high):
...
class DPSL(nn.Module):
    ...
    def forward(self, low_img, high_img):
        ...
        p_fused, entropy_loss = self.fusion(p_dfp_low, p_dfp_high)
        logits = self.final_fc(p_fused)
        return logits, p_dfp_low, p_dfp_high, entropy_loss
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for low_imgs, high_imgs, labels in train_loader:
            low_imgs = low_imgs.to(device)
            high_imgs = high_imgs.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits, p_dfp_low, p_dfp_high, entropy_loss = model(
                low_imgs, high_imgs
            )
        ...
        val_preds, val_trues = [], []
        with torch.no_grad():
            for low_imgs, high_imgs, labels in val_loader:
                low_imgs = low_imgs.to(device)
                high_imgs = high_imgs.to(device)
                labels = labels.to(device)

                logits, _, _, _ = model(low_imgs, high_imgs)
...
def test_model(model, test_loader, device):
    ...
    attentions = []
    with torch.no_grad():
        for low_imgs, high_imgs, labels in test_loader:
            low_imgs = low_imgs.to(device)
            high_imgs = high_imgs.to(device)
            labels = labels.to(device)
...
def main():
    ...
    full_dataset = DroneDataset(low_img_dirs, high_img_dirs, transform=transform)
